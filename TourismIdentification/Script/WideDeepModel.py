...
class DeepDenseModel(torch.nn.Module):
    def __init__(self, input_dim: int, hidden_layers: List[int], batchNorm=False, dropout: List[int] = None):
...
class WideDeepModel(torch.nn.Module):
    ...
    def __init(self, wide_model, deep_dense_model, output_dim: int = 1, deep_text_model=None, deep_head_layers=None,
        ...
        if self.deep_head is None:
            if head_layers_dims is not None:
                ...
                self.deep_dense = torch.nn.Sequential(self.deepdense,
                                                      torch.nn.Linear(self.deepdense.output_dim, output_dim))
                if self.deep_text is not None:
                    self.deep_text = torch.nn.Sequential(self.deeptext,
                                                         torch.nn.Linear(self.deeptext.output_dim, output_dim))
    ...
    def forward(self, X: List[Dict[str, Tensor]]) -> Tensor:
        ...
        if self.deep_head:
            ...
            deepside_out = self.deep_head(deep_side)
            return F.softmax(out.add_(deepside_out))
        else:
